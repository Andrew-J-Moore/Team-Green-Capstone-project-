...
export class GroupDataStorageService {
  private baseUrlGroup = "http://ec2-3-93-23-176.compute-1.amazonaws.com:8080/api/group/";
  ...
  createGroupWithFile(formData: FormData) {
    return this.http
      .post<ApiResponse>(
        "http://ec2-3-93-23-176.compute-1.amazonaws.com:8080/api/group/createFromFile",
        formData
      )
      .pipe(
        (map(data => data), catchError(error => throwError(error))),
  ...
  postMajors() {
    this.isLoadingSubject.next(true);
    return this.http
      .post<ApiResponse>("http://ec2-3-93-23-176.compute-1.amazonaws.com:8080/api/admin/", null)
      .pipe(
        (map(data => data), catchError(error => throwError(error))),
        finalize(() => this.isLoadingSubject.next(false))
      )
